...
sap.ui.define([
	...
	"sap/m/MessageBox",
	"industry/gov/au/fi/apinv/model/grouper",
	"industry/gov/au/fi/apinv/model/GroupSortState"
], function(BaseController, JSONModel, Filter, FilterOperator, GroupHeaderListItem, Device, formatter, MessageBox, grouper,
	GroupSortState) {
	"use strict";

	return BaseController.extend("industry.gov.au.fi.apinv.controller.Master", {
		...
		onOpenViewSettings: function() {
			if (!this._oViewSettingsDialog) {
				this._oViewSettingsDialog = sap.ui.xmlfragment("industry.gov.au.fi.apinv.view.ViewSettingsDialog", this);
		...
		onConfirmViewSettingsDialog: function(oEvent) {
			...
			aFilterItems.forEach(function(oItem) {
				switch (oItem.getKey()) {
					case "Filter1":
						aFilters.push(new Filter("Dmbtr", FilterOperator.LE, 5000));
						break;
					case "Filter2":
						aFilters.push(new Filter("Dmbtr", FilterOperator.GT, 5000));
						break;
					case "Filter3":
						aFilters.push(new Filter("Ftype", FilterOperator.EQ, "I"));
						break;
					case "Filter4":
						aFilters.push(new Filter("Ftype", FilterOperator.EQ, "C"));
						break;	
					case "Filter5":
				...
				this._oListSelector.oWhenListLoadingIsDone.then(
					function(mParams) {
						...
						if (!mParams.list.getSelectedItem()) {
							...
							this.getRouter().navTo("object", {
								Zreqid: encodeURIComponent(mParams.firstListitem.getBindingContext().getProperty("Zreqid"))
							}, true);
						}
					}.bind(this),
					function(mParams) {
		...
		_showDetail: function(oItem) {
			...
			this.getRouter().navTo("object", {
				...
				Zreqid: encodeURIComponent(oItem.getBindingContext().getProperty("Zreqid"))
			}, bReplace);
		},
