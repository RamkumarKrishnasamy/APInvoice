sap.ui.define([
	...
	return BaseController.extend("industry.gov.au.fi.apinv.controller.LineItem", {
		 ...
		 */
		_navBack: function() {
			/*Commented the code to resolve the closing the app immediately after done button pressed - RAM - 10.08.2018*/
		...
		_fnEntityCreated: function(oData) {
			var sObjectPath = this.getModel().createKey("InvoiceHeaderSet", oData);
		...
		_onObjectMatched: function(oEvent) {
			...
			var requestID = oParameter.Zreqid;

			var sObjectPath = "InvoiceItemsSet('" + requestID + "')";
			this._bindView("/" + sObjectPath);

		},
		formatLintItemAmount : function(oEvent) {
			var _oSource = oEvent.getSource();
			var sHeaderTotal = _oSource.getValue();
			if(sHeaderTotal ==="" || isNaN(sHeaderTotal)){
				sHeaderTotal = 0;
			}
			_oSource.setValue(parseFloat(sHeaderTotal).toFixed(2));
		},
		lineItemAmountChange : function(oEvent){
			var oTableItems = this._lineItemTable.getItems();
			var ocommonModel = this.getModel("commonModel");
			//headerTotalAmount
			var sHeaderTotal = parseFloat(this._headerContext.getProperty("Dmbtr"));
			if(sHeaderTotal ==="" || isNaN(sHeaderTotal)){
				sHeaderTotal = 0;
			}
			var lineItem = oEvent.getSource();
			...
			$.each(oTableItems, function(index, oTableItem) {
				...
				if(lineItem.getBindingContext()!== oTableItemContext){
			...
			if(lineItem.getValue()!==""){
				sLineItemsTotal += parseFloat(lineItem.getValue());	
			}
